...
async function fetchIncomingRequests() {
  ...
  try {
    ...
    if (response.ok === true) {
      ...
      if (pendingDonations.length === 0) {
        ...
        for (let i = 0; i < pendingDonations.length; i++) {
          ...
          // Build the card HTML
          card.innerHTML =
            '<div class="card-header-top"><h4>Food: ' +
            item.food_name +
            "</h4></div>" +
            '<div class="card-body-details">' +
            '<div class="detail-row"><strong>Quantity:</strong> ' +
            item.approx_quantity +
            "</div>" +
            '<div class="detail-row"><strong>Type:</strong> <span class="category-badge ' +
            category +
            '">' +
            category +
            "</span></div>" +
            ...
            (item.name || "Anonymous") +
            "</div>" +
            '<div class="detail-row"><strong>City:</strong> ' +
            item.city +
            "</div>" +
            "</div>" +
    ...
    container.innerHTML =
      '<p style="color:red;text-align:center;">Error loading requests. Please try again.</p>';
  }
}
