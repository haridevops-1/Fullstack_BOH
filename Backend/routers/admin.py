...
def stats(db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    verifiedRes = db.query(models.Trust).filter(models.Trust.is_verified == True).count()
...
def get_pending_trusts(db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
...
def verify_trust(id: int, action: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
...
def get_all_donors(db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
