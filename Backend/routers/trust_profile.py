...
def update_profile(
    trust_id: int,
    profile_data: schemas.TrustProfileUpdate,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user),
):
    ...
    if profile_data.trust_name:
        trust.trust_name = profile_data.trust_name
    if profile_data.email_id:
        trust.email_id = profile_data.email_id
    if profile_data.mobile_number:
        trust.mobile_number = profile_data.mobile_number
    if profile_data.trust_address:
        trust.trust_address = profile_data.trust_address
    if profile_data.city:
        trust.city = profile_data.city
    if profile_data.pincode:
        trust.pincode = profile_data.pincode
    if profile_data.trust_photo:
        trust.trust_photo = upload_image(profile_data.trust_photo)
