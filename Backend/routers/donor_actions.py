...
def dashboard(
    ...
    total = (
        db.query(models.Donation).filter(models.Donation.donor_id == donor_id).count()
    )
    pending = (
        db.query(models.Donation)
        .filter(
            models.Donation.donor_id == donor_id, models.Donation.status == "pending"
        )
        .count()
    )
    rejected = (
        db.query(models.Donation)
        .filter(
            models.Donation.donor_id == donor_id, models.Donation.status == "rejected"
        )
        .count()
    )
    accepted = (
...
def get_donations(
    ...
    results = (
        db.query(models.Donation, models.Trust.trust_name)
        .join(models.Trust, models.Donation.trust_id == models.Trust.id)
