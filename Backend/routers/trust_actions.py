...
def dashboard(trust_id: int, db: Session = Depends(get_db)):
...
def donations(trust_id: int, db: Session = Depends(get_db)):
...
def accepted_details(trust_id: int, db: Session = Depends(get_db)):
...
def rejected_details(trust_id: int, db: Session = Depends(get_db)):
    return db.query(models.Donation).filter(
        models.Donation.trust_id == trust_id, 
